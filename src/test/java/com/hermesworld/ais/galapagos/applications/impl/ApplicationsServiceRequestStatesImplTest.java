...
public class ApplicationsServiceRequestStatesImplTest {
	...
	public void feedCommonMocks() {
		...
		when(kafkaEnvironments.getGlobalRepository("known-applications", KnownApplicationImpl.class)).thenReturn(appRepository);
	}

	@Test
	public void testFromNothingToSubmitted() throws Exception {
		ApplicationsServiceImpl applicationServiceImpl = new ApplicationsServiceImpl(kafkaEnvironments, currentUserService,
			mock(TimeService.class),
			eventManager, mock(ApplicationsConfig.class));
	...
	public void testFromSubmittedToSubmitted() throws Throwable {
		...
		ApplicationOwnerRequest applicationOwnerRequest = createRequest(RequestState.SUBMITTED,
				now);
		repository.save(applicationOwnerRequest);

		ApplicationsServiceImpl applicationServiceImpl = new ApplicationsServiceImpl(kafkaEnvironments, currentUserService,
			mock(TimeService.class),
			eventManager, new ApplicationsConfig());
		try {
	...
	public void testFromSubmittedToRejected() throws Exception {
		...
		ApplicationsServiceImpl appsServImpl = new ApplicationsServiceImpl(kafkaEnvironments,
			currentUserService, mock(TimeService.class), eventManager,
			mock(ApplicationsConfig.class));
	...
	public void testFromApprovedToSubmitted() throws Throwable {
		ZonedDateTime now = ZonedDateTime.of(LocalDateTime.of(2020, 3, 25, 10, 0), ZoneOffset.UTC);
		ApplicationOwnerRequest applicationOwnerRequest = createRequest(RequestState.APPROVED,
				now);
		repository.save(applicationOwnerRequest);
		...
		ApplicationsServiceImpl applicationServiceImpl = new ApplicationsServiceImpl(kafkaEnvironments, currentUserService,
			...
			eventManager, mock(ApplicationsConfig.class));
		try {
			ApplicationOwnerRequest appOwnReq =
			applicationServiceImpl.submitApplicationOwnerRequest(applicationOwnerRequest.getApplicationId(), "").get();
	...
	public void testFromResignToSubmit() throws Exception {
		ZonedDateTime now = ZonedDateTime.of(LocalDateTime.of(2020, 3, 25, 10, 0), ZoneOffset.UTC);
		ApplicationOwnerRequest applicationOwnerRequest = createRequest(RequestState.RESIGNED,
				now);
		repository.save(applicationOwnerRequest);
	...
	public void testFromRevokedToSubmit() throws Exception {
		...
		ApplicationOwnerRequest applicationOwnerRequest = createRequest(RequestState.REVOKED,
				now);
		repository.save(applicationOwnerRequest);
	...
	public void testFromRejectedToSubmitted() throws Exception {
		ZonedDateTime now = ZonedDateTime.of(LocalDateTime.of(2020, 3, 25, 10, 0), ZoneOffset.UTC);
		ApplicationOwnerRequest applicationOwnerRequest = createRequest(RequestState.REJECTED,
				now);
		repository.save(applicationOwnerRequest);
