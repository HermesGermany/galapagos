...
public class UpdateApplicationAclsListenerTest {
	...
	public void testUpdateApplicationAcls() throws InterruptedException, ExecutionException {
		...
		UpdateApplicationAclsListener listener = new UpdateApplicationAclsListener(topicService,
		...
		assertNotNull("No DESCRIBE_CONFIGS right for cluster included",
			createdAcls.stream()
				.filter(b -> b.pattern().resourceType() == ResourceType.CLUSTER && b.pattern().patternType() == PatternType.LITERAL
					&& b.pattern().name().equals("kafka-cluster") && b.entry().permissionType() == AclPermissionType.ALLOW
		...
		assertNotNull(createdAcls.stream()
			.filter(binding -> binding.pattern().resourceType() == ResourceType.TOPIC
				&& binding.pattern().patternType() == PatternType.PREFIXED && binding.pattern().name().equals("de.myapp."))
			.findAny().orElse(null));
		assertNotNull(createdAcls.stream()
			.filter(binding -> binding.pattern().resourceType() == ResourceType.GROUP
				&& binding.pattern().patternType() == PatternType.PREFIXED && binding.pattern().name().equals("group.myapp."))
			.findAny().orElse(null));
		assertNotNull(createdAcls.stream()
			.filter(binding -> binding.pattern().resourceType() == ResourceType.GROUP
				&& binding.pattern().patternType() == PatternType.PREFIXED && binding.pattern().name().equals("de.myapp."))
		...
		assertNotNull(createdAcls.stream()
			.filter(binding -> binding.pattern().resourceType() == ResourceType.GROUP
				&& binding.pattern().patternType() == PatternType.PREFIXED && binding.pattern().name().equals("group2.myapp."))
			.findAny().orElse(null));
		...
		WRITE_TOPIC_OPERATIONS.forEach(op -> assertNotNull("Did not find expected write ACL for topic",
				createdAcls.stream()
						.filter(binding -> binding.pattern().resourceType() == ResourceType.TOPIC
								&& binding.pattern().patternType() == PatternType.LITERAL && binding.pattern().name().equals("topic1")
		...
		when(subscriptionService.getSubscriptionsOfApplication("_test", "app01", false)).thenReturn(Collections.singletonList(sub));
		...
		READ_TOPIC_OPERATIONS.forEach(op -> assertNotNull("Did not find expected read ACL for topic", createdAcls.stream()
			.filter(binding -> binding.pattern().resourceType() == ResourceType.TOPIC
				&& binding.pattern().patternType() == PatternType.LITERAL && binding.pattern().name().equals("topic2")
				&& binding.entry().operation() == op && binding.entry().principal().equals("User:CN=testapp"))
			.findAny().orElse(null)));
	}
	...
	public void testNoWriteAclsForInternalTopics() {
		...
		UpdateApplicationAclsListener listener = new UpdateApplicationAclsListener(topicService, subscriptionService, applicationsService);
