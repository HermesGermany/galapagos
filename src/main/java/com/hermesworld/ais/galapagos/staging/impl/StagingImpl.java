...
public final class StagingImpl implements Staging, ApplyChangeContext {
	...
	public static CompletableFuture<StagingImpl> build(String applicationId, String sourceEnvironmentId,
		...
		List<Change> changesFilter, TopicService topicService, SubscriptionService subscriptionService) {
		...
		try {
			compareSourceTarget(sourceTopics, targetTopics, TopicMetadata::getName,
				t -> {
		...
		List<SubscriptionMetadata> sourceSubs = subscriptionService.getSubscriptionsOfApplication(sourceEnvironmentId, applicationId,
			false);
		List<SubscriptionMetadata> targetSubs = subscriptionService.getSubscriptionsOfApplication(targetEnvironmentId, applicationId,
			false);
		compareSourceTarget(sourceSubs, targetSubs, StagingImpl::buildSubscriptionId, s -> changes.add(ChangeBase.subscribeTopic(s)),
			s -> changes.add(ChangeBase.unsubscribeTopic(s)), null);

		for (TopicMetadata topic : sourceTopics) {
			boolean isCreatedApiTopic = createdApiTopics.contains(topic.getName());
			List<SchemaMetadata> sourceSchemas = topicService.getTopicSchemaVersions(sourceEnvironmentId, topic.getName());
			List<SchemaMetadata> targetSchemas = topicService.getTopicSchemaVersions(targetEnvironmentId, topic.getName());
	...
	private static ApplicableChange checkForCreateTopic(TopicMetadata topic, String sourceEnvironmentId,
		...
		TopicCreateParams createParams = topicService.buildTopicCreateParams(sourceEnvironmentId, topic.getName()).join();
	...
	private static List<ApplicableChange> checkForTopicChanges(TopicMetadata oldTopic, TopicMetadata newTopic) {
		...
		if ((!oldTopic.isDeprecated() && newTopic.isDeprecated())
			|| (newTopic.isDeprecated() && !Objects.equals(oldTopic.getDeprecationText(), newTopic.getDeprecationText()))
			|| (newTopic.isDeprecated() && !Objects.equals(oldTopic.getEolDate(), newTopic.getEolDate()))) {
			result.add(ChangeBase.markTopicDeprecated(newTopic.getName(), newTopic.getDeprecationText(), newTopic.getEolDate()));
		}
		...
		if (oldTopic.isSubscriptionApprovalRequired() != newTopic.isSubscriptionApprovalRequired()) {
			result.add(
				ChangeBase.updateTopicSubscriptionApprovalRequiredFlag(newTopic.getName(), newTopic.isSubscriptionApprovalRequired()));
		}
	...
	private static <T> void compareSourceTarget(Collection<T> source, Collection<T> target,
		...
		Consumer<T> createdHandler, Consumer<T> deletedHandler, BiConsumer<T, T> checkForChangesHandler) {
		for (T tSrc : source) {
