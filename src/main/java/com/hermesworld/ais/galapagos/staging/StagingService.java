...
public interface StagingService {
	 ...
	 * </ul>
	 * If the optional <code>changesFilter</code> is provided and not empty, the result is filtered, and only changes equal
	 * to one of the changes in the given filter list are included. Note that this may lead to an inconsistent change set,
	 * which may not fully be applied to the target environment successfully. <br>
	 * <br>
	 ...
	 *
	 * @return A completable future providing the <code>Staging</code> object when done, or failing if the staging cannot be
	 * calculated for whatever reason.
	 */
	CompletableFuture<Staging> prepareStaging(String applicationId, String environmentIdFrom, List<Change> changesFilter);
