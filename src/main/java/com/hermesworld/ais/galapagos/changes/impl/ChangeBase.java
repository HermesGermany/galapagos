...
public abstract class ChangeBase implements ApplicableChange {
	 ...
	 * afterwards (in the order of the list). The returned future of the <code>applyTo</code> method of this change returns
	 * when <b>all</b> changes are complete, or fails if <b>any</b> change fails. Note that some changes may have been
	 * applied successfully to the target environment even in case of failure of the future.
	 *
...
final class SubscribeToTopicChange extends ChangeBase {
	...
	protected boolean isEqualTo(ChangeBase other) {
		...
		return Objects.equals(subscriptionMetadata.getClientApplicationId(), change.subscriptionMetadata.getClientApplicationId())
...
final class UpdateTopicDescriptionChange extends ChangeBase {
	...
	@Override
	public CompletableFuture<?> applyTo(ApplyChangeContext context) {
		return context.getTopicService().updateTopicDescription(context.getTargetEnvironmentId(), topicName, newDescription);
	}
...
final class UpdateSubscriptionApprovalRequiredFlagChange extends ChangeBase {
	...
	@Override
	public CompletableFuture<?> applyTo(ApplyChangeContext context) {
		return context.getTopicService().setSubscriptionApprovalRequiredFlag(context.getTargetEnvironmentId(), topicName,
