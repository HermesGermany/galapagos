	...
	private Change deserializeChange(JsonNode tree, ObjectMapper mapper) {
		...
		try {
			...
			case TOPIC_CREATED:
				...
				if (params == null) {
					return new V010CreateTopicChange(mapper.treeToValue(tree.findValue("topicMetadata"), TopicMetadata.class));
				}
				return ChangeBase.createTopic(mapper.treeToValue(tree.findValue("topicMetadata"), TopicMetadata.class),
						mapper.treeToValue(tree.findValue("createParams"), TopicCreateParamsDto.class).toTopicCreateParams());
			case TOPIC_DELETED:
				return ChangeBase.deleteTopic(tree.findValue(TOPIC_NAME).asText(), tree.findValue("internalTopic").asBoolean());
			case TOPIC_SUBSCRIBED:
				return ChangeBase
						.subscribeTopic(mapper.treeToValue(tree.findValue("subscriptionMetadata"), SubscriptionMetadata.class));
			case TOPIC_UNSUBSCRIBED:
				return ChangeBase
						.unsubscribeTopic(mapper.treeToValue(tree.findValue("subscriptionMetadata"), SubscriptionMetadata.class));
			case TOPIC_SUBSCRIPTION_UPDATED:
				return ChangeBase
						.updateSubscription(mapper.treeToValue(tree.findValue("subscriptionMetadata"), SubscriptionMetadata.class));
			...
			case TOPIC_DEPRECATED:
				return ChangeBase.markTopicDeprecated(tree.findValue(TOPIC_NAME).asText(),
						tree.findValue("deprecationText").asText(), mapper.treeToValue(tree.findValue("eolDate"), LocalDate.class));
		...
		log.error("Unsupported Change type in Changelog data: " + changeType + ". KafkaChangeDeserializer class must be updated.");
		return UNKNOWN_CHANGE;
