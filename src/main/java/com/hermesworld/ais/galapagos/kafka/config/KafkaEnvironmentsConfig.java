...
public class KafkaEnvironmentsConfig {
	...
	public KafkaClusters kafkaClusters(CertificateService certificateService, KafkaExecutorFactory executorFactory)
		...
		return new ConnectedKafkaClusters(environments.stream().collect(Collectors.toList()), caManagers, fTruststoreFile,
				productionEnvironment, metadataTopicsPrefix, executorFactory);
	}
	...
	private void validateConfig() {
		...
		if (!environments.stream().filter(env -> productionEnvironment.equals(env.getId())).findAny().isPresent()) {
			throw new RuntimeException("No environment configuration given for production environment " + productionEnvironment);
		}

		if (!certificatesWorkdir.isFile()) {
			throw new IllegalArgumentException("galapagos.kafka.certificates-workdir must point to a directory on the file system!");
		}
