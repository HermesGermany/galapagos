...
public class ConnectedKafkaClusters implements KafkaClusters {
	...
	public ConnectedKafkaClusters(List<KafkaEnvironmentConfig> environmentMetadata, Map<String, CaManager> caManagers,
		...
		for (KafkaEnvironmentConfig envMeta : environmentMetadata) {
			KafkaRepositoryContainerImpl repoContainer = new KafkaRepositoryContainerImpl(connectionManager, envMeta.getId(),
				galapagosInternalPrefix);
			ConnectedKafkaCluster cluster = buildConnectedKafkaCluster(envMeta.getId(), connectionManager, repoContainer,
				futureDecoupler);
	...
	private static ConnectedKafkaCluster buildConnectedKafkaCluster(String environmentId,
		...
		KafkaRepositoryContainer repositoryContainer, KafkaFutureDecoupler futureDecoupler) {
		return new ConnectedKafkaCluster(environmentId, repositoryContainer, connectionManager.getAdminClient(environmentId),
