...
public interface KafkaClusters {
	...
	default List<KafkaCluster> getEnvironments() {
		return getEnvironmentIds().stream().map(id -> getEnvironment(id).orElse(null))
	...
	/**
	 * Returns a repository for saving and retrieving objects in a Kafka Topic. Repositories returned by this method can be
	 * used to store information for use across all clusters (best example is the repository for Application Owner
	 * Requests). Usually, this repository will be stored in the production Kafka cluster, although clients should not rely
	 * on this and leave the exact storage location up to the implementation.
	 *
	 * @param <T> Type of the objects to store in the repository.
	 * @param topicName Name of the repository. It will be prefixed with a globally configured prefix for determining the
	 * "real" Kafka Topic name.
