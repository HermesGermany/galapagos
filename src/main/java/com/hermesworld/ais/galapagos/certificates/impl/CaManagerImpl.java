...
final class CaManagerImpl implements CaManager {
	...
	public CaManagerImpl(KafkaEnvironmentConfig environmentMetadata, File certificateWorkdir)
		...
		generateGalapagosPkcs12ClientCertificate(environmentMetadata.getId(), environmentMetadata.getClientDn(), certificateWorkdir);
	}
	...
	@Override
	public CompletableFuture<CertificateSignResult> createApplicationCertificateFromCsr(String applicationId, String csrData,
			String applicationName) {
		try (PEMParser parser = new PEMParser(new StringReader(csrData))) {
			Object o = parser.readObject();
			if (!(o instanceof PKCS10CertificationRequest)) {
				return CompletableFuture.failedFuture(new CertificateException("Invalid CSR data: no request found in data"));
			}
	...
	public CompletableFuture<CertificateSignResult> extendApplicationCertificate(String dn, String csrData) {
		...
		try (PEMParser parser = new PEMParser(new StringReader(csrData))) {
			...
			if (!parsedDn.equals(csr.getSubject())) {
				return CompletableFuture.failedFuture(new CertificateParsingException("The CSR is not valid for extending this certificate"));
			}
	...
	public CompletableFuture<CertificateSignResult> createDeveloperCertificateAndPrivateKey(String userName) {
		if (!supportsDeveloperCertificates()) {
			return CompletableFuture
				.failedFuture(new IllegalStateException("Developer certificates are not enabled for this environment."));
		}
		return createCertificateAndPrivateKey(userName, caData.getDeveloperCertificateValidity());
	}

	private CompletableFuture<CertificateSignResult> createCertificateAndPrivateKey(String userOrAppName, long validityMs) {
		...
		try {
			...
			return createCertificate(csr, cn, validityMs).thenCompose(result -> {
				try {
					return CompletableFuture
							.completedFuture(new CertificateSignResult(result.getCertificate(), result.getCertificatePemData(), result.getDn(),
									buildPrivateKeyStore(result.getCertificate(), pair.getPrivate(), "changeit".toCharArray())));
				}
				catch (OperatorCreationException | PKCSException e) {
					return CompletableFuture.failedFuture(new CertificateException("Could not generate internal CSR for certificate", e));
				}
			...
			return CompletableFuture.failedFuture(new RuntimeException("Java Security is configured wrong, or Bouncycastle not found"));
		}
	...
	private void generateGalapagosPkcs12ClientCertificate(String envId, String dn, File certificateWorkdir)
		...
		PKCS10CertificationRequest csr = buildCsr(name, pair);
		X509CertificateHolder holder = signCertificateRequest(csr, name, ChronoUnit.YEARS.getDuration().toMillis() * 10);
	...
	private CompletableFuture<CertificateSignResult> createCertificate(PKCS10CertificationRequest csr,
		...
		if (cn == null) {
			return CompletableFuture.failedFuture(new CertificateParsingException("No CN attribute present in Certificate Request. Please include CN="
				+ commonName + " as subject in the request."));
		}
		if (!cn.equals(commonName)) {
			return CompletableFuture.failedFuture(new CertificateParsingException(
				"Wrong CN in Certificate Request. Please include CN=" + commonName + " as subject in the request."));
		}
	...
	private CompletableFuture<CertificateSignResult> createCertificate(PKCS10CertificationRequest csr, X500Name dn,
		...
		return CompletableFuture.completedFuture(new CertificateSignResult(certificate, pemData.toString(), newDn, null));
	}
	...
	private X509CertificateHolder signCertificateRequest(PKCS10CertificationRequest csr, X500Name subject, long validityMs)
			throws GeneralSecurityException, OperatorCreationException {
		...
		try {
			...
			certGenerator.addExtension(Extension.authorityKeyIdentifier, false, utils.createAuthorityKeyIdentifier(caHolder));
	...
	private X500Name uniqueX500Name(String cn) {
		...
		rdns.add(new RDN(new AttributeTypeAndValue(X509ObjectIdentifiers.organizationalUnitName, new DERUTF8String(ouValue))));
	...
	private static CaData buildCaData(KafkaEnvironmentConfig env) throws IOException, GeneralSecurityException {
		...
		if (env.getCaCertificateFile() == null) {
			throw new RuntimeException("Missing configuration property caCertificateFile for environment " + env.getId());
		}
		...
		try (InputStream inPublic = env.getCaCertificateFile().getInputStream();
			...
			data.setCaCertificate((X509Certificate) CertificateFactory.getInstance("X.509", "BC").generateCertificate(inPublic));
			...
			if (!Arrays.equals(kp.getPublic().getEncoded(), data.getCaCertificate().getPublicKey().getEncoded())) {
				throw new RuntimeException(
						"The public/private key pair does not match for certificate " + data.getCaCertificate().getSubjectDN().getName());
			}
		...
		try {
			...
			log.warn("Invalid duration pattern found in application configuration: " + env.getApplicationCertificateValidity());
	...
	private static byte[] buildPrivateKeyStore(X509Certificate publicCertificate, PrivateKey privateKey, char[] password)
		...
		keyStoreBuilder.addEncryptedData(new JcePKCSPBEOutputEncryptorBuilder(PKCSObjectIdentifiers.pbeWithSHAAnd128BitRC2_CBC)
		...
		byte[] pkcs12Bytes = keyStoreBuilder.build(new JcePKCS12MacCalculatorBuilder(OIWObjectIdentifiers.idSHA1), password)
