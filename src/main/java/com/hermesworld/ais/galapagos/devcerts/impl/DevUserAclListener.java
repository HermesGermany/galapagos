...
public class DevUserAclListener implements TopicEventsListener, SubscriptionEventsListener, ApplicationEventsListener {
	...
	public CompletableFuture<Void> handleTopicSubscriptionApprovalRequiredFlagChanged(TopicEvent event) {
		...
		for (String appId : applicationIds) {
			...
			if (appMeta != null) {
				result = result.thenCompose(o -> updateAcls(cluster, getValidDevCertificatesForApplication(cluster, appId)));
			}
		}
	...
	public CompletableFuture<Void> handleTopicDeleted(TopicEvent event) {
		...
		Set<String> clientApplicationIds = subscriptionService
				...
				.map(s -> s.getClientApplicationId())
	...
	private Set<DevCertificateMetadata> getValidDevCertificatesForApplication(KafkaCluster cluster, String applicationId) {
		...
		return getRepository(cluster).getObjects().stream().filter(dev -> isValid(dev) && userNames.contains(dev.getUserName()))
				.collect(Collectors.toSet());
	}

	private Set<DevCertificateMetadata> getValidDevCertificateForUser(KafkaCluster cluster, String userName) {
		return getRepository(cluster).getObjects().stream().filter(dev -> isValid(dev) && userName.equals(dev.getUserName()))
				.collect(Collectors.toSet());
	}

	private boolean isValid(DevCertificateMetadata metadata) {
		return metadata.getExpiryDate() != null && metadata.getExpiryDate().isAfter(timeService.getTimestamp().toInstant());
	}
	...
	private class DevKafkaUser implements KafkaUser {
		...
		private Set<ApplicationMetadata> getApplicationsOfUser(String userName, String environmentId) {
			return applicationsService.getAllApplicationOwnerRequests().stream()
					...
					.map(req -> applicationsService.getApplicationMetadata(environmentId, req.getApplicationId()).orElse(null))
					.filter(m -> m != null).collect(Collectors.toSet());
		}

		private AclBinding toOwnAcl(AclBinding acl) {
			return new AclBinding(new ResourcePattern(acl.pattern().resourceType(), acl.pattern().name(), acl.pattern().patternType()),
					new AccessControlEntry(getKafkaUserName(), acl.entry().host(), acl.entry().operation(), acl.entry().permissionType()));
		}
