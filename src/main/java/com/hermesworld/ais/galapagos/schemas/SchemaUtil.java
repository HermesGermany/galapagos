...
public final class SchemaUtil {
	...
	private static void verifyCompatibleTo(Schema oldSchema, Schema newSchema, String prefix)
		...
		if (newSchema.getClass() == ObjectSchema.class) {
			...
			throw new IncompatibleSchemaException("Unsupported schema type used: " + oldSchema.getClass().getSimpleName());
		}
	}
	...
	private static void verifyCompatibleTo(ObjectSchema oldSchema, ObjectSchema newSchema, String prefix)
		...
		for (String property : oldSchema.getRequiredProperties()) {
			if (!newSchema.getRequiredProperties().contains(property)) {
				throw new IncompatibleSchemaException("Property " + fullPropName(prefix, property) + " is no longer required, but old consumers could rely on it.");
			}
		}
		...
		if (oldSchema.getMinProperties() != null && (newSchema.getMinProperties() == null
			...
			throw new IncompatibleSchemaException(
				"Minimum number of properties cannot be reduced at " + propLocationName(prefix)
					+ ". Old consumers could rely on having at least N properties.");
		}

		// Previously strongly typed (optional) properties must still be available with
		// same format
		oldPropsLoop:
		for (String property : oldSchema.getPropertySchemas().keySet()) {
		...
		for (Map.Entry<Regexp, Schema> pattern : oldPatternSchemas.entrySet()) {
			...
			Optional<Regexp> newKey = newPatternSchemas.keySet().stream().filter(r -> r.toString().equals(pattern.getKey().toString())).findAny();
			if (newKey.isEmpty()) {
	...
	private static void verifyCompatibleTo(ArraySchema oldSchema, ArraySchema newSchema, String prefix)
		...
		if (oldSchema.getAllItemSchema() != null) {
			...
			if (newSchema.getItemSchemas() != null) {
				...
				if (newSchema.getItemSchemas().size() > oldSchema.getItemSchemas().size() && (!oldSchema.permitsAdditionalItems() ||
					(oldSchema.getMaxItems() != null
						&& oldSchema.getMaxItems() < newSchema.getItemSchemas().size()))) {
					throw new IncompatibleSchemaException("New schema defines more items in array at " + propLocationName(prefix) + " than were previously allowed here.");
				}
				...
				if (newSchema.getMinItems() == null || newSchema.getMinItems() < oldSize) {
					throw new IncompatibleSchemaException("Old schema defined " + oldSize + " items for array at " + propLocationName(prefix) + ", but new schema does not guarantee this number of items in the array");
				}
				...
				for (int i = 0; i < oldSchema.getItemSchemas().size(); i++) {
					verifyCompatibleTo(oldSchema.getItemSchemas().get(i), newSchema.getAllItemSchema(), prefix + "[" + i + "]");
				}
			}
			else {
				throw new IncompatibleSchemaException(
	...
	private static void verifyCompatibleTo(StringSchema oldSchema, StringSchema newSchema, String prefix)
			throws IncompatibleSchemaException {
		if (oldSchema.getMinLength() != null && (newSchema.getMinLength() == null
			|| newSchema.getMinLength() < oldSchema.getMinLength())) {
			throw new IncompatibleSchemaException(
				"New schema allows for shorter strings than old schema for string property "
					+ propLocationName(prefix));
		}
		if (oldSchema.getMaxLength() != null && (newSchema.getMaxLength() == null
