...
public interface TopicService {
	...
	/**
	 * Marks the given topic as deprecated. Unlike most other operations, this is done automatically on <b>all</b>
	 * known clusters (environments) where the topic exists.
	 *
	 * @param topicName       Name of the topic.
	 * @param deprecationText Deprecation text, e.g. a reasoning and a reference to an alternative topic.
	 * @param eolDate         EOL date until when the topic is guaranteed to exist. The topic can only be deleted before this
	 *                        date when it has no more subscribers (standard topic deletion rules and order apply).
	 * @return A future which completed once the topic has been deprecated on all known clusters, or which completes
	 * exceptionally when the deprecation could not be performed successfully. Note that the topic may be deprecated
	 * on <b>some</b> stages even if the future completes exceptionally.
	 */
	@CheckReturnValue
	CompletableFuture<Void> markTopicDeprecated(String topicName, String deprecationText, LocalDate eolDate);

	/**
	 * Removes the deprecation mark from the given topic. Unlike most other operations, this is done automatically
	 * on <b>all</b> known clusters (environments) where the topic exists.
	 *
	 * @param topicName Name of the topic.
	 * @return A future which completed once the topic has been undeprecated on all known clusters, or which completes
	 * exceptionally when the deprecation removal could not be performed successfully. Note that the topic may be
	...
	/**
	 * Updates the "subscriptionApprovalRequired" flag for a given topic on a given environment. This is a stageable change,
	 * and can only be performed on non-"staging only" environments. <br>
	 * If the flag is activated for a topic, all already existing subscriptions on that environment for the topic will
	 * initially receive the <code>APPROVED</code> state. This can be updated using the <code>SubscriptionService</code>.
	 * <br>
	 * Once it is deactivated for a topic, all <code>PENDING</code> subscriptions for the topic on the given environment
	 * will be converted to <code>APPROVED</code> ones (this is not done by the <code>TopicService</code>, but by the class
	 * <code>SubscriptionTopicListener</code>).
	 *
	 * @param environmentId                ID of the environment containing the topic. Must not be a "staging only" environment.
	 ...
	 * object, which is usually used during staging only. This method checks that the preceding version of the schema exists
	 * on the given environment, e.g. you can add a schema version #3 only if #2 is already existing. If this condition is
	 * not fulfilled, a failed <code>CompletableFuture</code> (with an {@link IllegalStateException} as cause) is returned.
	 *
	 ...
	 * @return A Future which completes when the operation completes, or a failed future when the schema could not be added.
	 */
	@CheckReturnValue
