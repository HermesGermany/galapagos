...
public class TopicServiceImpl implements TopicService, InitPerCluster {
	...
	private static final Comparator<SchemaMetadata> schemaVersionsComparator = Comparator.comparingInt(SchemaMetadata::getSchemaVersion);
	...
	@Override
	public CompletableFuture<TopicMetadata> createTopic(String environmentId, TopicMetadata topic, Integer partitionCount,
			Map<String, String> topicConfig) {
		KnownApplication ownerApplication = applicationsService.getKnownApplication(topic.getOwnerApplicationId()).orElse(null);
		if (ownerApplication == null) {
			return CompletableFuture
				.failedFuture(new IllegalArgumentException("Unknown application ID: " + topic.getOwnerApplicationId()));
		}
		...
		ApplicationMetadata metadata = applicationsService.getApplicationMetadata(environmentId, topic.getOwnerApplicationId())
			.orElse(null);
		if (metadata == null) {
			return CompletableFuture
				.failedFuture(new IllegalArgumentException(
					"Application " + topic.getOwnerApplicationId() + " is not registered on environment " + environmentId));
		}

		if (!applicationsService.isUserAuthorizedFor(metadata.getApplicationId())) {
			return CompletableFuture
					.failedFuture(new IllegalStateException("Current user is no owner of application " + metadata.getApplicationId()));
		}
		...
		return environment.getActiveBrokerCount().thenCompose(brokerCount -> {
			...
			TopicBasedRepository<TopicMetadata> topicRepository = environment.getRepository(METADATA_TOPIC_NAME, TopicMetadata.class);
	...
	@Override
	public CompletableFuture<Void> updateTopicDescription(String environmentId, String topicName, String newDescription) {
		return doWithClusterAndTopic(environmentId, topicName, (kafkaCluster, metadata, eventSink) -> {
			...
			return getTopicRepository(kafkaCluster).save(newMeta).thenCompose(o -> eventSink.handleTopicDescriptionChanged(newMeta));
		});
	}
	...
	public CompletableFuture<Void> markTopicDeprecated(String topicName, String deprecationText, LocalDate eolDate) {
		return doDeprecationAction(topicName, (kafkaCluster, metadata, eventSink) -> {
			...
			return getTopicRepository(kafkaCluster).save(newMeta).thenCompose(o2 -> eventSink.handleTopicDeprecated(newMeta));
		});
	}
	...
	public CompletableFuture<Void> unmarkTopicDeprecated(String topicName) {
		return doDeprecationAction(topicName, (kafkaCluster, metadata, eventSink) -> {
			...
			return getTopicRepository(kafkaCluster).save(newMeta).thenCompose(o2 -> eventSink.handleTopicUndeprecated(newMeta));
		});
	}
		...
		return doWithClusterAndTopic(environmentId, topicName, (kafkaCluster, metadata, eventSink) -> {
			...
			if (metadata.getType() == TopicType.INTERNAL) {
				return CompletableFuture.failedFuture(
						new IllegalStateException("Cannot update subscriptionApprovalRequired flag for application internal topics"));
			}
	...
	public List<TopicMetadata> listTopics(String environmentId) {
		...
		return getTopicRepository(kafkaCluster).getObjects().stream().sorted(topicsComparator).collect(Collectors.toList());
	}
	...
	public List<SchemaMetadata> getTopicSchemaVersions(String environmentId, String topicName) {
		...
		return getSchemaRepository(kafkaCluster).getObjects().stream().filter(meta -> topicName.equals(meta.getTopicName()))
			.sorted(schemaVersionsComparator).collect(Collectors.toList());
	}

	@Override
	public Optional<SchemaMetadata> getSchemaById(String environmentId, String schemaId) {
		return kafkaClusters.getEnvironment(environmentId).flatMap(cluster -> getSchemaRepository(cluster).getObject(schemaId));
	}
	...
	public CompletableFuture<Void> deleteLatestTopicSchemaVersion(String environmentId, String topicName) {
		...
		if (existingVersions.isEmpty()) {
			return CompletableFuture.failedFuture(new IllegalStateException("No Schemas on current stage for topic "
				+ topicName));
		}
		SchemaMetadata latestSchemaOnCurrentStage = existingVersions.get(existingVersions.size() - 1);

		SchemaMetadata schemaOnNextStage = getTopicSchemaVersions(nextEnvId, topicName)
			.stream().filter(v -> latestSchemaOnCurrentStage.getSchemaVersion() == v.getSchemaVersion())
		...
		if (schemaOnNextStage != null) {
			return CompletableFuture.failedFuture(new IllegalStateException("The selected schema already exists on the next stage! To delete " +
	...
	public CompletableFuture<SchemaMetadata> addTopicSchemaVersion(String environmentId,
		...
		if (metadata.getType() == TopicType.INTERNAL) {
			return CompletableFuture.failedFuture(new IllegalStateException("Cannot add JSON schemas to internal topics"));
		}
		...
		if (newSchema.definesProperty("data") && (metadata.getType() == TopicType.EVENTS || metadata.getType() == TopicType.COMMANDS)) {
		...
		if (existingVersions.isEmpty() && schemaMetadata.getSchemaVersion() != 1) {
			return CompletableFuture.failedFuture(new IllegalArgumentException(
				"Illegal next schema version number #" + schemaMetadata.getSchemaVersion() + " for topic " + topicName));
		}
		if (!existingVersions.isEmpty()
			&& existingVersions.get(existingVersions.size() - 1).getSchemaVersion() != schemaMetadata.getSchemaVersion() - 1) {
			return CompletableFuture.failedFuture(new IllegalArgumentException(
				"Illegal next schema version number #" + schemaMetadata.getSchemaVersion() + " for topic " + topicName));
		}
		
		SchemaMetadata previousVersion = existingVersions.isEmpty() ? null : existingVersions.get(existingVersions.size() - 1);
		...
		if (previousVersion != null) {
			try {
				...
				if (SchemaUtil.areEqual(newSchema, previousSchema)) {
					return CompletableFuture
						.failedFuture(new IllegalArgumentException("The new schema is identical to the latest schema of this topic."));
				}

				boolean reverse = metadata.getType() == TopicType.COMMANDS;
				SchemaUtil.verifyCompatibleTo(reverse ? newSchema : previousSchema, reverse ? previousSchema : newSchema);
			}
			catch (JSONException e) {
				// how, on earth, did it get into the repo then???
				log.error("Invalid JSON schema in repository found for topic " + topicName + " on environment " + environmentId
						+ " with schema version " + previousVersion.getSchemaVersion());
	...
	public CompletableFuture<List<ConsumerRecord<String, String>>> peekTopicData(String environmentId, String topicName,
		...
		if (metadata.getType() == TopicType.INTERNAL) {
			return CompletableFuture
				.failedFuture(new IllegalStateException("Data of internal topics cannot be retrieved via Galapagos."));
		}
	...
	private CompletableFuture<Void> doWithClusterAndTopic(String environmentId, String topicName, TopicServiceAction action) {
		KafkaCluster kafkaCluster = kafkaClusters.getEnvironment(environmentId).orElse(null);
		if (kafkaCluster == null) {
			return FutureUtil.noSuchEnvironment(environmentId);
		}
	...
	private CompletableFuture<Void> doDeprecationAction(String topicName, TopicServiceAction action) {
		...
		environmentIds = environmentIds.stream().filter(env -> kafkaClusters.getEnvironment(env)
			.map(cluster -> getTopicRepository(cluster).containsObject(topicName)).orElse(false))
			.collect(Collectors.toList());

		if (environmentIds.isEmpty()) {
			return CompletableFuture.failedFuture(new NoSuchElementException("Topic " + topicName + " not found on any environment"));
		}
		...
		for (String envId : environmentIds) {
			TopicServiceAction wrappedAction = (cluster, metadata, sink) -> action.apply(cluster, metadata, eventSinks.get(envId));
