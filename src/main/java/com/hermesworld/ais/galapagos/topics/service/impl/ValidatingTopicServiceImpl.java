...
public class ValidatingTopicServiceImpl implements ValidatingTopicService {
	...
	public ValidatingTopicServiceImpl(@Qualifier(value = "nonvalidating") TopicService topicService,
		SubscriptionService subscriptionService, ApplicationsService applicationsService, KafkaClusters kafkaClusters,
		GalapagosTopicConfig topicConfig) {
	...
	public boolean canDeleteTopic(String environmentId, String topicName) {
		...
		if (!subscriptionService.getSubscriptionsForTopic(environmentId, topicName, false).isEmpty() && !isEolDatePast) {
			return false;
		}
	...
	public CompletableFuture<Void> deleteLatestTopicSchemaVersion(String environmentId, String topicName) {
		...
		if (!subscriptionService.getSubscriptionsForTopic(environmentId, topicName, false).isEmpty()) {
			return CompletableFuture.failedFuture(new IllegalStateException("Schemas of subscribed Topics cannot be deleted!"));
		}
	...
	public CompletableFuture<Void> markTopicDeprecated(String topicName, String deprecationText, LocalDate eolDate) {
		if (eolDate.isBefore(LocalDate.now().plus(topicConfig.getMinDeprecationTime()))) {
			return CompletableFuture.failedFuture(new IllegalArgumentException("EOL date for deprecated topic must be at least " +
	...
	public CompletableFuture<Void> setSubscriptionApprovalRequiredFlag(String environmentId, String topicName,
		...
		return checkOnNonStaging(environmentId, "update subscriptionApprovalRequired flag").orElseGet(
			() -> topicService.setSubscriptionApprovalRequiredFlag(environmentId, topicName, subscriptionApprovalRequired));
	}
	...
	private <T> Optional<CompletableFuture<T>> checkOnNonStaging(String environmentId, String action, Class<T> resultClass) {
		if (kafkaClusters.getEnvironmentMetadata(environmentId).map(KafkaEnvironmentConfig::isStagingOnly).orElse(false)) {
	...
	public CompletableFuture<List<ConsumerRecord<String, String>>> peekTopicData(String environmentId, String topicName,
		...
		if (metadata != null && metadata.isSubscriptionApprovalRequired() && !currentUserMayRead(environmentId, metadata)) {
	...
	private boolean currentUserMayRead(String environmentId, TopicMetadata metadata) {
		Set<String> subscribedApplications = subscriptionService.getSubscriptionsForTopic(environmentId, metadata.getName(), false).stream()
