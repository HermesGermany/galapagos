...
public class ApplicationsController {
	...
	public List<KnownApplicationDto> listUserApplications() {
		return applicationsService.getUserApplications().stream().map(app -> toKnownAppDto(app)).collect(Collectors.toList());
	}
	...
	@PutMapping(value = "/api/me/requests", consumes = MediaType.APPLICATION_JSON_VALUE,
			produces = MediaType.APPLICATION_JSON_VALUE)
	public ApplicationOwnerRequest submitApplicationOwnerRequest(@RequestBody ApplicationOwnerRequestSubmissionDto request) {
		if (StringUtils.isEmpty(request.getApplicationId())) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Required parameter applicationId is missing or empty");
		}
		applicationsService.getKnownApplication(request.getApplicationId())
				.orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));

		try {
			return applicationsService.submitApplicationOwnerRequest(request.getApplicationId(), request.getComments()).get();
		}
	...
	@PostMapping(value = "/api/admin/requests/{id}", consumes = MediaType.APPLICATION_JSON_VALUE,
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Secured("ROLE_ADMIN")
	...
	public List<ApplicationCertificateDto> getApplicationCertificates(@PathVariable String applicationId) {
		return kafkaClusters.getEnvironmentsMetadata().stream().map(env -> applicationsService
				.getApplicationMetadata(env.getId(), applicationId).map(meta -> toAppCertDto(env.getId(), meta)).orElse(null))
				.filter(dto -> dto != null).collect(Collectors.toList());
	}

	@PostMapping(value = "/api/certificates/{applicationId}/{environmentId}", consumes = MediaType.APPLICATION_JSON_VALUE,
			produces = MediaType.APPLICATION_JSON_VALUE)
	public CertificateResponseDto updateApplicationCertificate(@PathVariable String applicationId,
		...
		applicationsService.getKnownApplication(applicationId).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));

		if (!applicationsService.isUserAuthorizedFor(applicationId)) {
			throw new ResponseStatusException(HttpStatus.FORBIDDEN);
		}

		kafkaClusters.getEnvironmentMetadata(environmentId).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
	...
	private CertificateResponseDto createCertificateAndKey(String environmentId, String applicationId, String topicPrefix) {
		try {
	...
	private KnownApplicationDto toKnownAppDto(KnownApplication app) {
		return new KnownApplicationDto(app.getId(), app.getName(), app.getInfoUrl() == null ? null : app.getInfoUrl().toString(),
				toCapDtos(app.getBusinessCapabilities()), new ArrayList<>(app.getAliases()));
	}

	private List<BusinessCapabilityDto> toCapDtos(List<? extends BusinessCapability> caps) {
		return caps.stream().map(cap -> new BusinessCapabilityDto(cap.getId(), cap.getName())).collect(Collectors.toList());
	}
