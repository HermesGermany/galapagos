...
/**
 * A service which is able to calculate the due reminders for application topic owners about the expiry of one of
 * their certificates. The service ensures that one reminder is reported to be sent (each), when
 * <ul>
...
public interface CertificateExpiryReminderService {
	 ...
	 *
	 * @return A list of reminders which should be sent out by the caller. Reminders may only occur in this list until they
	 * have been marked as sent out by a call to {@link #markReminderSentOut(CertificateExpiryReminder)}.
	 */
	List<CertificateExpiryReminder> calculateDueCertificateReminders();

	/**
	 * Informs the repository that a reminder (which has been returned by
	 * {@link #calculateDueCertificateReminders()}) has been sent out successfully and should no longer be
	 * returned as being due.
	 *
