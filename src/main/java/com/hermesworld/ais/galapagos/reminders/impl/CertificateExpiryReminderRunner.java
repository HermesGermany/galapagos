...
/**
 * Scheduling component which checks every six hours for new certificate expiry notifications to be sent (first check
 * is performed five minutes after application startup).
 */
@Component
@Slf4j
public class CertificateExpiryReminderRunner {
	...
	public CertificateExpiryReminderRunner(CertificateExpiryReminderService reminderService,
		NotificationService notificationService, ApplicationsService applicationsService, KafkaClusters kafkaClusters) {
	...
	public void checkCertificatesForExpiration() {
		...
		for (CertificateExpiryReminder reminder : reminders) {
			try {
				...
				log.error("Could not send out certificate expiration reminder e-mail for application " + reminder.getApplicationId(),
					e.getCause());
	...
	private CompletableFuture<Void> sendReminderEmail(CertificateExpiryReminder reminder) {
		NotificationParams params = new NotificationParams("certificate-reminder-" + reminder.getReminderType().name().toLowerCase(Locale.US));
	...
	private ZonedDateTime getDateOfExpiry(String applicationId, String environmentId) {
		return applicationsService.getApplicationMetadata(environmentId, applicationId).map(meta -> meta.getCertificateExpiresAt()).orElse(null);
	}
