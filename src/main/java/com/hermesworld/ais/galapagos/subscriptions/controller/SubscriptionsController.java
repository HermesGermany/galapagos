...
public class SubscriptionsController {
	...
	@GetMapping(value = "/api/applications/{applicationId}/subscriptions/{environmentId}",
	...
	public List<SubscriptionDto> getApplicationSubscriptions(@PathVariable String applicationId,
		...
		return subscriptionService.getSubscriptionsOfApplication(environmentId, applicationId, includeNonApproved).stream()
				.map(sub -> toDto(environmentId, sub))
				.collect(Collectors.toList());
	}

	@PutMapping(value = "/api/applications/{applicationId}/subscriptions/{environmentId}",
			consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	public SubscriptionDto createApplicationSubscription(@PathVariable String applicationId, @PathVariable String environmentId,
			@RequestBody CreateSubscriptionDto createData) {
		KafkaEnvironmentConfig environment = kafkaEnvironments.getEnvironmentMetadata(environmentId).orElseThrow(notFound);
		if (!applicationsService.isUserAuthorizedFor(applicationId) || environment.isStagingOnly()) {
			throw new ResponseStatusException(HttpStatus.FORBIDDEN);
		}

		try {
			SubscriptionMetadata subscription = subscriptionService
				.addSubscription(environmentId, createData.getTopicName(), applicationId, createData.getDescription()).get();
	...
	@PostMapping(value = "/api/topics/{environmentId}/{topicName}/subscriptions/{subscriptionId}",
			consumes = MediaType.APPLICATION_JSON_VALUE)
	public void updateApplicationSubscription(@PathVariable String environmentId, @PathVariable String topicName,
			@PathVariable String subscriptionId, @RequestBody UpdateSubscriptionDto updateData) throws InterruptedException {
	...
	public void deleteApplicationSubscription(@PathVariable String applicationId, @PathVariable String environmentId,
		...
		KafkaEnvironmentConfig environmentMeta = kafkaEnvironments.getEnvironmentMetadata(environmentId).orElseThrow(notFound);
	...
	public List<SubscriptionDto> getTopicSubscriptions(@PathVariable String environmentId, @PathVariable String topicName,
			@RequestParam(name = "includeNonApproved", defaultValue = "false") boolean includeNonApproved) {
		kafkaEnvironments.getEnvironmentMetadata(environmentId).orElseThrow(notFound);
