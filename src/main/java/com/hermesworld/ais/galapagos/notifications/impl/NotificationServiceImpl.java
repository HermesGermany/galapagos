...
public class NotificationServiceImpl implements NotificationService {
	...
	public CompletableFuture<Void> notifySubscribers(String environmentId, String topicName,
		...
		List<SubscriptionMetadata> subscriptions = subscriptionService.getSubscriptionsForTopic(environmentId, topicName, false);

		Set<String> applicationIds = subscriptions.stream().map(SubscriptionMetadata::getClientApplicationId).collect(Collectors.toSet());
		...
		for (String applicationId : applicationIds) {
			recipients.addAll(requests.stream().filter(req -> req.getState().equals(RequestState.APPROVED) && applicationId.equals(req.getApplicationId()) && !finalExcludeUser.equals(req.getUserName()))
				.map(ApplicationOwnerRequest::getNotificationEmailAddress).filter(s -> !StringUtils.isEmpty(s)).collect(Collectors.toSet()));
		}
	...
	@Override
	public CompletableFuture<Void> notifyRequestor(ApplicationOwnerRequest request, NotificationParams notificationParams) {
		...
		try {
			return doSendAsync(notificationParams, Arrays.asList(InternetAddress.parse(request.getNotificationEmailAddress())), false);
		}
		catch (AddressException e) {
	...
	public CompletableFuture<Void> notifyTopicOwners(String environmentId, String topicName,
		...
		String ownerApplicationId = topicService.getTopic(environmentId, topicName).map(m -> m.getOwnerApplicationId()).orElse(null);
		if (ownerApplicationId == null) {
			return CompletableFuture
				.failedFuture(new IllegalArgumentException("Unknown topic " + topicName + " on environment " + environmentId));
		}
